  ...
  useEffect(() => {
    ...
    inView("#experiences", () => {
      ...
      animate(
        nav,
        {
          backgroundColor: darkBlueNavbarBackground,
          color: textLightColor,
          opacity: 1,
        },
        animateOption,
      );
      makeActive(navPointer, document.querySelector('a[href="#experiences"]'));
    });

    inView("#contact", () => {
      console.log("#contact");
      animate(nav, { opacity: 0 }, animateOption);
      makeActive(navPointer, document.querySelector('a[href="#contact"]'));
    });
    ...
    function cleanActive() {
      nav
        ...
        ?.forEach?.((tab: any) => tab.classList.remove("active"));
    }
